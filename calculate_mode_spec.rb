require_relative "calculate_mode"

describe 'calculate_mode' do
  let(:array_1) { [1, 2, 3, 4, 5, 5, 7] }
  let(:array_2) { [4, 4, 5, 5, 6, 6, 6, 7, 5] }
  let(:array_3) { ["apple", "banana", "clementine", "banana", "cherry", "strawberry", "cherry"]}

  it "is defined" do
    expect(defined? calculate_mode).to eq 'method'
  end

  it "requires a single argument" do
    expect(method(:calculate_mode).arity).to eq 1

  end

  it "returns the correct mode when mode is a unique integer" do
    expect(calculate_mode(array_1)).to eq [5]
  end

  it "returns the correct mode when mode is not a unique integer" do
    expect(calculate_mode(array_2)).to eq [5,6]

  end

  it "returns the correct mode for strings" do
    expect(calculate_mode(array_3)).to eq ["banana", "cherry"]
  end
end
